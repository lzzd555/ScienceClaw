...
4. 候选发现不能只依赖 `csrf`、`token` 等字段名规则，还要支持高熵动态值扫描和 consumer-first 回溯。
5. 发布 MCP 时把高置信 token 关系保存为 `auth_flow` 规则，不保存真实 token 明文。
6. MCP runtime 调用 tool 前自动执行 token setup/extract，并把新 token 注入目标请求。
7. 当目标请求返回常见认证失败状态时，自动刷新 token 并重试一次。
8. UI 展示“系统检测到动态 token 流程”，允许用户确认或关闭。
...
Producer 发现器从以下来源提取候选 token。候选发现不能只靠字段名白名单，否则会漏掉业务系统自定义的字段，例如 `r`、`guard`、`x-company-state`、`x-rpa-auth`。第一版应采用“显式语义规则 + 高熵动态值扫描”的双层策略。

### 6.1 两层候选策略

第一层是显式语义规则，用字段名快速捕获常见 token。这一层精度高，适合直接给出较强信号。

第二层是高熵动态值扫描，不要求字段名像 token。只要某个响应、DOM、storage 或 cookie 中出现的字符串满足动态值特征，就可以进入候选池。动态值特征包括：

- 长度足够，默认大于等于 8。
- 有一定字符复杂度，不是纯短数字、布尔值、常见枚举或普通状态文本。
- 不明显是业务展示值，例如用户名、标题、状态名、短 ID。
- 出现在 response/DOM/storage/cookie 中，且后续请求中出现同值或规范化后同值。
- 多次录制中重复呈现相同 producer -> consumer 关系时提高置信度。

示例：

```json
{
  "r": "8fa7c91e2d8a4c90b0f7"
}
```

即使字段名 `r` 没有语义，只要后续请求出现：

```text
X-Company-Guard: 8fa7c91e2d8a4c90b0f7
```

系统也应通过值流匹配生成 token flow。字段名只影响命名和置信理由，不应是进入候选池的唯一条件。
...
如果 key 不包含这些语义词，但 value 满足高熵动态值特征，也应进入候选池，等待后续 consumer 匹配确认。
...
Analyzer 应同时支持 producer-first 和 consumer-first 两条路径。

Producer-first：

1. 从 response/DOM/storage/cookie 中发现候选动态值。
2. 在后续 request headers/query/body/cookies 中查找同值。
3. 找到后建立 producer -> consumer flow。

Consumer-first：

1. 先扫描后续 API request 中所有非噪声位置，例如自定义 header、敏感 query、JSON body 字段、form body 字段。
2. 对请求值做动态值过滤，挑出高熵或字段名可疑的 consumer candidate。
3. 回头在该请求之前的 response/DOM/storage/cookie 候选池中查找同值或规范化同值。
4. 找到 source 后建立 producer -> consumer flow。

Consumer-first 用于补足字段名不明显的 producer。例如响应字段名是 `r`，但后续请求 header 是 `X-Company-Guard`；如果 producer-first 因字段名不明显没有强信号，consumer-first 仍可以从后续请求的自定义 header 值回溯到前面的响应值。

两条路径的最终裁决仍然依赖真实值匹配和时间顺序。AI 不参与匹配裁决。
...
熵过滤只能决定是否进入候选池，不能单独证明 token flow。只有观察到 source 和 consumer 的实际值流向后，才可以生成中高置信 flow。
...
- response JSON 高熵字段 `r` -> request header `X-Company-Guard`
- consumer-first 从自定义 request header 回溯到前置 response 字段
...
- 支持显式字段名规则、高熵动态值扫描和 consumer-first 回溯。
...
- token 名字不明显，但值流向明确。这应通过高熵值扫描和 consumer-first 回溯解决，命名可能需要 AI 或规则辅助。
